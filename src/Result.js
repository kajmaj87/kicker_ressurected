...
const Result = (props) => {
    ...
    const readKickerRanking = async function () {
        ...
        ranking.data.map(r => {
          r.wins = matches.data.reduce((acc, match) => (match.winners.indexOf(r.user) >= 0 ? 1 : 0) + acc);
          r.loses = matches.data.reduce((acc, match) => (match.losers.indexOf(r.user) >= 0 ? 1 : 0) + acc);
        });
